...
in
{
  ...
  config =
    let
      isNullOrEmpty =
        v: v == null || ((lib.isString v) && v == "") || ((lib.isList v) && builtins.length v == 0);
    ...
    in
    {
      systemd.services = forAllTargets "rclone-backup" (
        target: targetCfg:
            ...
            in
            {
              ...
              ExecStart =
                let
                  ...
                  transfers =
                    if targetCfg.transfers == 4 then "" else "--transfers ${builtins.toString targetCfg.transfers}";
      ...
      systemd.timers = forAllTargets "rclone-backup-timer" (
        target: targetCfg:
        lib.mkIf targetCfg.enable {
