		...
		systemd.services = forAllTargets "rsync-backup" (target: targetCfg:
			lib.mkIf targetCfg.enable {
				...
				in {
					Type = "oneshot";
					ExecStart = ''
						${lib.getExe targetCfg.package} -a --no-perms --chmod=Du=rwx,Dg=rwx,Fu=rw,Fg=rw ----chown=:labmembers ${rsyncDeleteFlag} --partial '${targetCfg.sourceDir}/' '${targetCfg.destDir}/'
					'';
				} // optionalConfigs;
			}
		);
