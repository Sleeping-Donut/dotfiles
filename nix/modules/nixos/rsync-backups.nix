		...
		systemd.services = forAllTargets "rsync-backup" (target: targetCfg:
			lib.mkIf targetCfg.enable {
				...
				in {
					Type = "oneshot";
					ExecStart = ''
						${lib.getExe targetCfg.package} -a --no-perms --chmod=Du=rwx,Dg=rwx,Fu=rw,Fg=rw --chown=:labmembers --omit-dir-times --partial ${rsyncDeleteFlag} '${targetCfg.sourceDir}/' '${targetCfg.destDir}/'
					'';
				} // optionalConfigs;
			}
		);
